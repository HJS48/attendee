...
def validate_zoom_oauth_connections_sync(zoom_oauth_app_id, retry_count=0):
    """
    Synchronous version of validate_zoom_oauth_connections for direct execution without Celery.
    Used in Kubernetes mode where Celery worker is not available.
    """
    MAX_RETRIES = 1
    RETRY_BACKOFF_BASE = 2

    logger.info(f"Validating zoom oauth connections for zoom oauth app {zoom_oauth_app_id} (sync)")
...
def enqueue_validate_zoom_oauth_connections_task(zoom_oauth_app_id: int):
    """Enqueue a validate zoom oauth connections task for a zoom oauth app."""
    from bots.task_executor import is_kubernetes_mode, task_executor

    if is_kubernetes_mode():
        task_executor.submit(validate_zoom_oauth_connections_sync, zoom_oauth_app_id)
    else:
        validate_zoom_oauth_connections.delay(zoom_oauth_app_id)
